    ...
    <Link href={`/${lang}`} aria-label="Home" className={containerStyles[variant]} >
      <div className={`relative ${variantStyles[variant]}`}>
        <svg 
          ...
          viewBox="0 0 599 279" 
          className={`w-full h-full ${variant === 'footer' ? 'text-txcolor-muted' : ''}`}
        >
          <g>
            <path 
              className={`${variant === 'footer' ? 'fill-current' : 'fill-prcolor'}`}
            ...
            />
            <path 
              className={`${variant === 'footer' ? 'fill-current' : 'fill-accolor'}`}
